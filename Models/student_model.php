...
function student_add_data($data) {
    ...
    foreach ($subjects as $subject) {
        ...
        $student_id = mysqli_query($connection, $last_student);
        // var_dump($student_id);die();
        $query_subject = "INSERT INTO student_has_subjects(student_id, subjects_id) VALUES($student_id,intval($subject)";
...
function m_update_data($data) {
    ...
    include "connection.php";
    $query = "UPDATE student SET firstname = '$firstname', lastname='$lastname', class = '$class', sex='$sex' FROM student INNER JOIN subjects SET sub_title='$subject' ON (student_id = subjects_id) WHERE id='$id'";
    $result = mysqli_query($connection, $query);
    return $result;
}
